"""A2CAgent: Advantage Actor-Critic (A2C) agent for discrete action spaces."""
...
class A2CAgent:
    ...
    def __init__(self, env: gym.Env):
        ...
        #    list(self._policy_net.parameters()) + list(self._value_net.parameters()), lr=self.LR
        # )
    ...
    def train(self) -> None:
        ...
        for episode in range(self.NUM_EPISODES):
            states, actions, next_states, rewards = self._run_episode()
            self._update(states, actions, next_states, rewards)
    ...
    def _run_episode(self) -> tuple[list[np.ndarray], list[int], list[np.ndarray], list[float]]:
    ...
    def _compute_td_targets(self, next_states: list[np.ndarray], rewards: list[float]) -> torch.Tensor:
        ...
        targets = [r + self.GAMMA * v_next.item() for r, v_next in zip(rewards, next_values)]
        targets[-1] = rewards[-1]  # terminal step: no bootstrapping

        return torch.tensor(targets, dtype=torch.float32)

    def _update(
        self, states: list[np.ndarray], actions: list[int], next_states: list[np.ndarray], rewards: list[float]
    ) -> None:
        states_tensor = torch.from_numpy(np.array(states)).float()
        actions_tensor = torch.tensor(actions, dtype=torch.long)

        td_targets = self._compute_td_targets(next_states, rewards)
