    ...
    def train(self) -> tuple[int, dict[State, float], dict[State, Action]]:
        ...
        for _ in range(self._max_iters):
            ...
            for i in range(self._max_steps):
                ...
                # TD error: δ = R + γ·V(s') - V(s)  (V(s') = 0 if s' is terminal)
                v_next = 0.0 if self._env.is_terminal(next_state) else values.get(next_state, 0.0)
                td_error = reward + self._gamma * v_next - values.get(state, 0.0)

                # Critic update: w ← w + α^w · δ · ∇v̂(s, w)
                values[state] = values.get(state, 0.0) + self._alpha_critic * td_error

                # Actor update: θ ← θ + α^θ · I · δ · ∇ln π(A|s, θ)
                # Full softmax gradient: ∂ln π(A|s)/∂h(s,a) = 1{a=A} - π(a|s)
        ...
        best_policy = utils.calc_best_policy_from_quality(self._preferences)
