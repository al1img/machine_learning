...
class QLearningAgent:
    """Q-learning agent to find the optimal policy for a given GridWorld environment."""
    ...
    def train(self) -> tuple[int, dict[State, float], dict[State, Action]]:
        ...
        for _ in range(self._max_iters):
            ...
            for i in range(self._max_steps):
                ...
                q[state][action] += self._alpha * (
                    reward + self._gamma * max(q[next_state].values(), default=0.0) - q[state][action]
                )
